...
func connectToDatabase() *sql.DB {
	db, err := sql.Open("mysql", "root@tcp(127.0.0.1:3306)/prayer")
	if err != nil {
		log.Fatal(err)
	}
	return db
...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query("SELECT times_id, fajr, dhuhr, asr, maghrib, isha FROM timings")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var times_id, fajr, dhuhr, asr, maghrib, isha string

		// Scan the selected values into the variables
		err = rows.Scan(&times_id, &fajr, &dhuhr, &asr, &maghrib, &isha)
		if err != nil {
			log.Fatal(err)
		}
